...
class UsersBeanTest {
    ...
    private Document[] generateUsers() {
        return new Document[]{
                generateUser(new ObjectId("000000000000000000000000"), "bla@blabla.de", "blabla", "pass1234"),
    ...
    void failRegisterTest(String comment, String email, String username, String password, String expectedException) {
        ...
        try {
            assertThrows((Class<? extends Throwable>) Class.forName(expectedException),
                    () -> user[0] = bean.register(email, username, password));
        }
    ...
    void loginTest(String comment, String email, String username, String password, String login, String loginPassword,
        ...
        if(correct) {
            ...
            Class<? extends Throwable> exception =
                    complete ? InvalidLoginException.class : IncompleteRequestException.class;
    ...
    @ParameterizedTest(name = "[{index}] {0}")
    @CsvFileSource(resources = "/users/login.csv")
    void loginWithTokenTest(String comment, String email, String username, String password, String login,
            String loginPassword, boolean correct, boolean complete) throws InterruptedException {
        Document doc = generateUser(new ObjectId(), email, username, password);
        container.getCollection("users").insertOne(doc);
        ...
        if(correct) {
            ...
            assertEquals(doc.getObjectId("_id"), user.getUser().getID());
            Document tokenDoc = container.getCollection("tokens").find(Filters
                    .and(Filters.eq("userId", user.getUser().getID()), Filters.eq("token", user.getToken()))).first();
